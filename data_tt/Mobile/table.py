...
prepaid_url = base_url + "/particulier/mobile/internet-mobile" # Changer le lien pour chaque sous-rubrique

# Liste des offres existantes
offers = [
    "internet-صب-ة" # Changer les offres pour chaque sous-rubrique
]
...
for offer in offers:
    ...
    try:
        ...
        results.append({
            "URL": full_url,
            "RUBRIQUE": "Mobile",
            "SOUS-RUBRIQUE": "Internet Mobile", # Changer la sous-rubrique pour chaque sous-rubrique
            "Offre": title.get_text(strip=True) if title else offer.upper(),
            "Contenu": page_content if page_content else "Contenu non trouvé",
            "Tableau": tables_data if tables_data else "Aucun tableau trouvé"
        })
